...
Evaluates TSCD across 7 diverse domains using the 10-layer CNN:
  Medical:      BreakHis
  Agricultural: PlantVillage
  Remote Sensing: EuroSAT
  Industrial:   NEU Surface Defect
  Astronomical: Galaxy10
  Texture:      DTD
  Fine-grained: Food-101

Usage:
    python -m experiments.train_domain --dataset eurosat
    python -m experiments.train_domain --dataset all
"""
...
def run_single(dataset_name: str, output_dir: str, hp: dict, seed: int = 42):
    """Train TSCD on one domain-specific dataset."""
    ...
    print(f"\n  {dataset_name}: best acc = {best:.2f}%\n")
    return history


def main():
    parser = argparse.ArgumentParser(
        description="TSCD — Table 8: Cross-Domain Generalization"
    )
    parser.add_argument("--dataset", type=str, default="eurosat",
                        help="Dataset name or 'all'.")
    parser.add_argument("--output_dir", type=str, default="./outputs/table8")
    ...
    datasets = DOMAIN_DATASETS if args.dataset == "all" else [args.dataset]
    ...
    print("\n" + "=" * 60)
    print("  Table 8 — Cross-Domain Generalization Results")
    print("=" * 60)
